...
export default function DetailedAnalysisModal({
  ...
  const [selectedBusinessType, setSelectedBusinessType] = useState<ShopType | undefined>(
    recommendation.businessRecommendations[0]?.type
  );
  const [activeTab, setActiveTab] = useState<'overview' | 'business' | 'rent' | 'risk'>('overview');
  ...
  const getTabIcon = (iconType: string) => {
    switch (iconType) {
      ...
      case 'store':
        return <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 3L2 12h3v8h14v-8h3L12 3zm0 2.69L18.31 12H16v6h-3v-4h-2v4H8v-6H5.69L12 5.69z"/></svg>;
      case 'money':
        return <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-1.41 6.41L16 14.17 14.83 15.41l-4.24-4.24-2.83 2.83L6.59 12.83 10.59 8.41z"/></svg>;
      case 'warning':
        return <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l-10 18h20L12 2zm-1 13h2v2h-2v-2zm0-6h2v4h-2V9z"/></svg>;
      default:
        return null;
    }
  };
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/50 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-6xl max-h-[90vh] overflow-hidden animate-scale-in">
        ...
        <div className="bg-gradient-primary text-white p-6 relative">
          ...
          <div className="pr-12">
            <h2 className="text-2xl font-bold mb-2">상권 분석 보고서</h2>
            <p className="text-white/90">{shopName}</p>
            <p className="text-sm text-white/80">{recommendation.location.address}</p>
            
            {/* 투자 매력도 */}
            <div className="mt-4 flex items-center space-x-4">
              <div className="bg-white/20 rounded-lg p-3">
                <div className="text-xs text-white/80">투자 매력도</div>
                <div className="text-2xl font-bold">{recommendation.investmentAttractiveness}</div>
              </div>
              <div className="text-sm text-white/90 flex items-center">
                {recommendation.investmentAttractiveness >= 80 ? (
                  <><svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> 매우 우수</>
                ) : recommendation.investmentAttractiveness >= 70 ? (
                  <><svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg> 우수</>
                ) : recommendation.investmentAttractiveness >= 60 ? (
                  <><svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"/></svg> 양호</>
                ) : recommendation.investmentAttractiveness >= 50 ? (
                  <><svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg> 보통</>
                ) : (
                  <><svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 24 24"><path d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg> 주의</>
                )}
              </div>
            </div>
          </div>
        ...
        <div className="p-6 overflow-y-auto max-h-[calc(90vh-200px)]">
            ...
            <div className="space-y-6">
              ...
              <div className="card">
                ...
                <div className="flex flex-wrap gap-3">
                  ...
                  {recommendation.targetCustomers.secondary.map((target, index) => (
                    <div key={index} className="bg-gray-100 px-3 py-2 rounded-lg">
                      <span className="text-sm text-gray-700">서브: {target}</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          {activeTab === 'business' && (
            <div className="space-y-6">
              ...
              {/* 추천 업종 상세 */}
              <div className="card">
                <h3 className="text-lg font-bold text-gray-900 mb-4">추천 업종 상세 분석</h3>
                <div className="space-y-4">
                  {recommendation.businessRecommendations.map((rec, index) => (
                    <div key={rec.type} className="border border-gray-200 rounded-lg p-4">
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-gray-500">추천 점수: {rec.score}/100</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <div className="text-sm text-gray-500">예상 성공률</div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-3">
                        <div className="bg-gray-50 p-3 rounded-lg text-center">
                          ...
                          <div className="text-xs text-gray-500">평균 월매출</div>
                        </div>
                        <div className="bg-gray-50 p-3 rounded-lg text-center">
                          <div className="text-lg font-bold text-gray-900">
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg text-center">
                          ...
                          <div className="text-xs text-gray-500">진입 난이도</div>
                        </div>
                      </div>
                      
                      <div className="space-y-2">
                        <h5 className="font-medium text-gray-900">추천 근거:</h5>
                        {rec.reasons.map((reason, reasonIndex) => (
                          <p key={reasonIndex} className="text-sm text-gray-600 flex items-start">
          ...
          {activeTab === 'rent' && (
            <div className="space-y-6">
              ...
              {/* 시세 변동 추이 */}
              <div className="card">
                <h3 className="text-lg font-bold text-gray-900 mb-4">최근 시세 변동 추이</h3>
                <div className="space-y-4">
                  <div className="flex justify-between items-center text-sm text-gray-600 mb-2">
                    <span>최근 6개월</span>
                    <span>평균 임대료 (만원)</span>
                  </div>
                  {recommendation.marketAnalysis.marketPrice.priceHistory.map((history, index) => {
                    const maxPrice = Math.max(...recommendation.marketAnalysis.marketPrice.priceHistory.map(h => h.averageRent));
                    const width = (history.averageRent / maxPrice) * 100;
                    
                    return (
                      <div key={history.month} className="flex items-center space-x-4">
                        <div className="w-16 text-sm text-gray-600">{history.month}</div>
          ...
          {activeTab === 'risk' && (
            <div className="space-y-6">
              ...
              <div className="card">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div className="bg-gray-50 p-4 rounded-lg">
                    <h4 className="font-semibold text-gray-900 mb-3">위험 완화 방안</h4>
                    <div className="space-y-2">
                      {[
                        "차별화된 컨셉으로 경쟁력 확보",
                        "다양한 수익 모델 구축",
              ...
              {/* 투자 수익률 시뮬레이션 */}
              <div className="card">
                <h3 className="text-lg font-bold text-gray-900 mb-4">투자 수익률 시뮬레이션</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[
                    { scenario: "낙관적", roi: "12.5%", color: "#10b981" },
                    { scenario: "보통", roi: `${recommendation.rentRecommendation.expectedROI}%`, color: "#6366f1" },
                    { scenario: "비관적", roi: "6.2%", color: "#ef4444" }
                  ].map((scenario) => (
                    <div key={scenario.scenario} className="bg-gray-50 p-4 rounded-lg text-center">
                      <div className="text-sm text-gray-600 mb-1">{scenario.scenario} 시나리오</div>
        ...
        <div className="border-t border-gray-200 p-4 bg-gray-50">
          <div className="flex justify-between items-center text-xs text-gray-500">
            <div>
              분석 생성 일시: {recommendation.generatedAt.toLocaleString('ko-KR')}
            </div>
